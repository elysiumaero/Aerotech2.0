...
"""
╔══════════════════════════════════════════════════════════════╗
║           RADHA GCS — Ground Control Station v1.0           ║
...
#    "mode": "HOVER", "armed": true, "ts": 1234567890}
#
# ── ESP32 → GCS  (ACK) ──────────────────────────────────────
#   {"ack": "ARM",  "status": "OK"}
...
class RADHAApp:
    ...
    def _build_flight_panel(self, parent):
        ...
        for i, (lbl, key, unit) in enumerate(rows):
            ...
            self._tv[key] = v
            tk.Label(g, textvariable=v, bg=PANEL, fg=TEXT,
                     font=F_BIG, anchor="w").grid(row=i, column=1, sticky="w", padx=8)
    ...
    def _add_seg(self):
        try:
            ...
            self._log("Invalid segment values", WARN); return
    ...
    def _on_ack(self, pkt):
        ...
        self.root.after(0, self._log, f"ACK {cmd}: {st} {msg}".strip(), col)

    def _on_conn_status(self, msg, col):
        def _do():
            self._conn_lbl.config(text=f"● {msg}", fg=col)
            connected = "CONNECTED" in msg and "DIS" not in msg
